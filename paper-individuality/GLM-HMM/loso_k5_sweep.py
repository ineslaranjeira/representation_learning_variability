"""
Leave-one-session-out (LOSO) sweep for K=5 only, using 3 random restarts per
...
Identical design to loso_k2_k3_sweep.py / loso_k4_sweep.py (same mice, same
held-out folds, same train-derived stimulus z-scoring and null baseline) so
results merge directly with loso_k2_k3_results.csv / loso_k4_results.csv for
a fair K5-vs-K2/K3/K4 comparison - this is a genuine held-out evaluation,
not the non-LOSO full-data fit used for state characterization elsewhere.

Run with the `glmhmm` conda env (projected ~35-45 min, based on the measured
32.3 min for the K=4 LOSO sweep):
    /opt/anaconda3/envs/glmhmm/bin/python3 loso_k5_sweep.py
"""
import time
import numpy as np
...
RESULTS_PATH = OUT_DIR / 'loso_k5_results.csv'
N_RESTARTS = 3
K = 5
