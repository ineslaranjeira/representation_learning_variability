...
#
# COST OF ASKING FOR MANY AT ONCE: intersecting all four leaves 19 mice, and
# the measured effects shrink monotonically as the cohort grows (Late vs
# Proficient: rho 0.23 at n=19, 0.14 at n=35, 0.08 at n=49) -- the small-cohort
# numbers are inflated. For any pair you actually want to report, re-run with
# just that pair so the cohort, and the power, is as large as it can be.
USE_TIMEPOINTS = ['Early', 'Late', 'Pre-rec', 'Proficient']

METRIC       = 'correlation'  # RDM dissimilarity: 'correlation' or 'euclidean'
K_NN         = 3              # neighbourhood size; keep k/(n-1) small or the test is coarse
