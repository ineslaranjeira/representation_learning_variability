...
def cross_validate_model(model, key, train_emissions,  method, num_train_batches, num_iters=100):
    # Initialize the parameters using K-Means on the full training set
    init_params, props = model.initialize(key=key, method=method, emissions=train_emissions)
    ...
    # Baseline model has the same number of states but random initialization
    def _fit_fold_baseline(y_train, y_val):
        return model.marginal_log_prob(init_params, y_val) /len(y_val)
    
    # Then actually fit the model to data
    def _fit_fold(y_train, y_val):
        fit_params, train_lps, ll_check, args = model.fit_em(init_params, props, y_train, y_train,
                                             num_iters=num_iters, verbose=False)
        return model.marginal_log_prob(fit_params, y_val) /len(y_val), fit_params, ll_check, args

    val_lls, fit_params, ll_check, args = vmap(_fit_fold)(folds, train_emissions)
    
    baseline_val_lls = vmap(_fit_fold_baseline)(folds, train_emissions)
    
    return val_lls, fit_params, init_params, baseline_val_lls, ll_check, args
    
"""

def cross_validate_model(model, key, all_emissions, train_emissions, method, num_train_batches, num_iters=100):
    # Initialize the parameters using K-Means on the full training set
    init_params, props = model.initialize(key=key, method=method, emissions=train_emissions)
    ...
    # Baseline model has the same number of states but random initialization
    def _fit_fold_baseline(y_val):
        
        fold_num = np.shape(y_val)[0]
        baseline_ll = np.ones(fold_num)
        for f, fold in enumerate(range(fold_num)):
            baseline_ll[f] = model.marginal_log_prob(init_params, np.shape(y_val[f])) /len(y_val[f])
        return baseline_ll
    ...
    # Baseline model has the same number of states but random initialization
    def _fit_fold_baseline(y_val):
        return model.marginal_log_prob(init_params, y_val) /len(y_val[0])
    ...
    def _fit_fold(y_train, y_val):
        fit_params, train_lps = model.fit_em(init_params, props, y_train,
                                                num_iters=num_iters, verbose=False)
        return model.marginal_log_prob(fit_params, y_val) /len(y_val[0]), fit_params
    ...
    baseline_val_lls = vmap(_fit_fold_baseline)(train_emissions)
